...
public class Alog {
	...
	private static boolean add( List<Node> list, int id, int size ) {
		...
		if( i == list.size() ) return false; // 不存在未分配且大于或等于size的内存空间节点
		...
		// 将原来节点变成已分配的节点,当剩余空间大于SIZE时,该节点大小为size;当剩余空间小于或等于SIZE时,该节点大小不变;
		p.id = id; p.size = (p.size-size>SIZE?size:p.size);
